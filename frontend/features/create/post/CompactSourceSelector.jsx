...
function CompactSourceSelector({ query, setSource, selectedSource }) {
    ...
    return (
        <FlatList
            ...
            renderItem={({item}) =>
                <Pressable onPress={() => setSource(item)} className={`transition-transform duration-150 ${item.id === selectedSource?.id ? 'translate-x-3 border-b border-rac' : 'hover:translate-x-1' } `}>
